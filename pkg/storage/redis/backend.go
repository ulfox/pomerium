...
const (
	...
	indexCIDRKey     = redisutil.KeyPrefix + "index_cidr"
...
func (backend *Backend) Get(ctx context.Context, recordType, id string) (_ *databroker.Record, err error) {
	...
	return getRecord(ctx, backend.client, recordType, id)
}
		...
		func(p redis.Pipeliner, version uint64) error {
			for _, record := range records {
				if record.DeletedAt != nil {
					deleteRecord(ctx, p, record.GetType(), record.GetId())
					deleteRecordIndexCIDR(ctx, p, record.GetType(), record.GetId())
				} else {
					setRecord(ctx, p, record)
					if cidr := storage.GetRecordIndexCIDR(record.GetData()); cidr != nil {
						setRecordIndexCIDR(ctx, p, record.GetType(), record.GetId(), *cidr)
					}
				}
				addRecordChange(ctx, p, record)
			}
