...
// AddrToFloat converts a net address into a floating point number.
func AddrToFloat(addr netip.Addr) float64 {
	if addr.Is4() {
		bs := addr.As4()
	...
	f, _ := big.NewFloat(0).SetInt(big.NewInt(0).SetBytes(addr.AsSlice())).Float64()
	return f
}
